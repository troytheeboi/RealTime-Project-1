...
void CashierProcess(int TperItem, int qid, long normMessage, long leaveMessage, struct Cashier *Cashier_arr, int cahierSem, int order)
{
    ...
    while (1)
    {
        ...
        if ((toLeave = msgrcv(qid, &notifier, notifierLenghth, leaveMessage, IPC_NOWAIT)) != -1)
        { // if the cashier recieves a leave message (which is type leave message) clean up resources and leave
        ...
        if ((result = msgrcv(qid, &customer, messageLenghth, normMessage, IPC_NOWAIT)) != -1)
        { // recieve a customer from the queue  with normal type means it was sent from actual customer
            printf("cashier recieved customer %d \n", customer.customer_id);

            if (is_process_alive(customer.customer_id))
            { // checks if customer process whose info is in customer struct is still alive
